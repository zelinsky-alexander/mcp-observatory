#!/usr/bin/env python3
"""Install and verify compact Storage-v2 indexes used by hot public list routes."""
...
INDEXES = {
    ...
    "analysis_runs_status_started": """
        ...
        ON analysis_runs(status, started_at COLLATE BINARY DESC, id DESC)
    """,
}

RECORDS_SQL = """SELECT id FROM server_versions
ORDER BY COALESCE(updated_at,published_at,'') COLLATE BINARY DESC, id DESC LIMIT 50"""
ANALYSES_SQL = """SELECT ar.id FROM analysis_runs ar WHERE ar.status='completed'
ORDER BY ar.started_at COLLATE BINARY DESC, ar.id DESC LIMIT 50"""
ALL_SERVERS_SQL = """WITH matching AS (
    SELECT id,server_identifier,updated_at,published_at,
           ROW_NUMBER() OVER (PARTITION BY server_identifier
             ORDER BY COALESCE(updated_at,published_at,'') COLLATE BINARY DESC, id DESC) AS rn
    FROM server_versions)
SELECT id FROM matching WHERE rn=1
ORDER BY COALESCE(updated_at,published_at,'') COLLATE BINARY DESC,
         server_identifier COLLATE BINARY LIMIT 50"""
...
def status(database: Path) -> dict[str, object]:
    ...
    try:
        ...
        index_names = {
            str(row["name"])
            for table in ("server_versions", "analysis_runs")
