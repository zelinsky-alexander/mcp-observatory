...
class AutomaticRuntimeResolverTests(unittest.TestCase):
    def test_default_runtime_prefers_node22(self) -> None:
        ...
        self.assertEqual(image, "node:22-trixie-slim")
        self.assertTrue(any("node:22-trixie-slim" in argv for argv in seen))

    def test_engine_range_selects_highest_satisfying_compatible_runtime(self) -> None:
        original = auto.base.run_docker
    ...
    def test_fixed_image_remains_supported(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            guard = Path(temporary) / "guard"
            guard.write_bytes(b"guard")
            self.assertEqual(
                auto.resolve_runtime_image(
